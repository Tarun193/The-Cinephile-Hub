...
const Movies = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      const response = await API.get(
        `/discover/movie?api_key=${API_KEY}&language=en-US&sort_by=popularity.desc&page=${currentPage}`
      );
  ...
  return (
    <main className="sw-5/6 md:w-2/3 mx-auto my-4">
      <h2 className="text-white text-3xl mb-4">Explore Movies</h2>
      <hr className="w-full h-[0.1rem] bg-white" />
      <ExploreGallery Content={movies} />
      <ReactPaginate
        breakLabel=".."
        nextLabel=""
